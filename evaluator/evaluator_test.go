...
func TestEvalIntegerExpression(t *testing.T) {
	tests := []struct {
		...
		{"-5 * -5", 25},
		{"100 / 5 / 2", 10},
		//{"100 / (5 / 2)", 40}, rounds the integer lol
		{"(2 + 3) * (4 + 1)", 25},
		{"(2 + 3) * 4 + 1", 21},
		{"2 + 3 * 4 + 1", 15},
		{"2 * (3 + 4 * 2)", 22},
...
func TestStringLiteral(t *testing.T) {
	...
	if str.Value != "Hello World!" {
		t.Errorf("String has wrong value, got=%q (%+v)", evaluated, evaluated)
	}
...
func TestBuiltinFunctions(t *testing.T) {
	test := []struct {
		...
		{`len([1,3,"miauw",90])`, 4},
		{`len([1,3,"miauw",90])`, 4},
		{`let x = [0,1,2,3,4,5]; first(x)`, 0},
		{`let x = ["miauw",1,2,3,4,5]; last(x)`, 5},
		{`first([1, 2, 3])`, 1},
		{`first([])`, nil},
	...
	for _, tt := range test {
		...
		case int:
			if !testIntegerObject(t, evaluated, int64(expected)) {
				t.Errorf("Expected integer object, but type=%s, object=%s", evaluated.Type(), evaluated.Inspect())
			}
		case nil:
			testNullObject(t, evaluated)
		case string:
		...
		case []int:
			...
			if len(array.Elements) != len(expected) {
				t.Errorf("wrong num of elements. want=%d, got=%d, input=%s",
					len(expected), len(array.Elements), tt.input)
				continue
			}
...
func TestArrrayIndexExpressions(t *testing.T) {
	tests := []struct {
		input    string
		expected any
	}{
	...
	input := `let two = "two";
{
	...
	if len(result.Pairs) != len(expected) {
		t.Fatal("len result.Pairs not correct!")
	}

	for k, v := range expected {
		exp, ok := result.Pairs[k]
		if !ok {
